...
Define the falsifiable gates each Copilot-native skill wrapper must satisfy. Wrappers are **thin projections**: they reference canonical skills, prompts, templates, and validators. They do not duplicate canonical content and do not encode review logic.

## Wrapper Set Under Test

| # | Wrapper ID | Adapter Wrapped | Skill Required |
|---|---|---|---|
| 1 | `aisraf-orchestration` | `.agents/aisraf-orchestrator.agent.md` | No (orchestrator owns no skill) |
| 2 | `aisraf-input-read` | `.agents/aisraf-input-reader.agent.md` | Yes |
| 3 | `aisraf-dfd-extraction` | `.agents/aisraf-dfd-extractor.agent.md` | Yes |
| 4 | `aisraf-review-table-build` | `.agents/aisraf-review-table-builder.agent.md` | Yes |
| 5 | `aisraf-blueprint-questioning` | `.agents/aisraf-blueprint-questioner.agent.md` | Yes |
| 6 | `aisraf-finding-recommendation` | `.agents/aisraf-finding-recommender.agent.md` | Yes |
| 7 | `aisraf-handoff-qa-score` | `.agents/aisraf-handoff-qa-scorer.agent.md` | Yes |

## Gates — Discovery (D1–D7)

Each wrapper must be discoverable by the operator surface (Copilot Chat customizations panel and/or Claude Code skill list).

| # | Gate | Falsifiable Check |
|---|---|---|
| D1 | `aisraf-orchestration` discoverable | Wrapper file exists at the approved canonical path; operator surface lists it by display name. |
...
| # | Gate | Falsifiable Check |
|---|---|---|
| R1 | Wrapper references the right adapter | Wrapper file contains the literal `.agents/aisraf-<id>.agent.md` path; SHA-256 of that adapter is unchanged from BP06 acceptance hash. |
...
| R7 | No canonical rewrite | No SK-*.md, prompt, PRA, or template body text is duplicated inline in any wrapper (line-grep against canonical bodies returns no full-line copies). |
...
| W3 | `aisraf-dfd-extraction` writes only `runs/{{run_id}}/02-visible-dfd-objects.md`, `03-legend-normalization.md`, `04-components.md`, `05-flows.md`, `06-boundaries.md`, and `runs/{{run_id}}/dfd/01-..-09-*.md`. | Same. |
| W4 | `aisraf-review-table-build` writes only `runs/{{run_id}}/07-security-stack-assessment.md`, `08-internal-review-table.md`, `09-missing-facts.md`. | Same. |
| W5 | `aisraf-blueprint-questioning` writes only `runs/{{run_id}}/10-ai-action-level.md`, `11-blueprint-match.md`, `12-targeted-questions.md`. | Same. |
