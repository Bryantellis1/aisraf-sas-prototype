...
> **`BP12-SAMPLE-DFD-BLOCKER: RESOLVED-BY-10A-AND-11A` (full resolution).** Build Package 10A reworked the canonical DFD and the affected expected baselines (default 4-token flow-tuple standard with explicit `AZ?`; embedded `subgraph LEGEND` panel inside the DFD; `<class> • R# • <Enc|Tok|Mask|Clr|Unknown>` data-store grammar). Build Package 11A byte-refreshed `runs/RUN-001/inputs/` from the corrected sample. The §6 defect-non-normalization gate PASSes because the inputs no longer carry the original defect. See `validation/package-10a-corrective-patch-checklist.md` and `validation/package-11a-input-refresh-checklist.md` for the falsifiable evidence. BP13 (Diagrams) is now `next_allowed`. The verdict text below is preserved as the BP12 historical record.
