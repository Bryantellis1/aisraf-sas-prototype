...
> **`BP12-SAMPLE-DFD-BLOCKER: RESOLVED-BY-10A-AND-11A` (full resolution).** Build Package 10A reworked the canonical DFD into a realistic GCP-style architecture review diagram with the default 4-token flow-tuple standard and embedded legend; Build Package 11A byte-refreshed `runs/RUN-001/inputs/` from the corrected sample. The diagrammatic-quality gates 7, 8, 11, 12, 13, 14, 15, 16 below now PASS against both `samples/sample-001-dfd-crop/inputs/` and `runs/RUN-001/inputs/`. See `validation/package-10a-corrective-patch-checklist.md` and `validation/package-11a-input-refresh-checklist.md` for the falsifiable evidence. BP13 (Diagrams) is now `next_allowed`. The verdict text below is preserved as the BP12 historical record.
