...
> **`BP12-SAMPLE-DFD-BLOCKER: RESOLVED-BY-10A-AND-11A` (full resolution).** Build Package 10A reworked `samples/sample-001-dfd-crop/inputs/dfd-crop.mmd` and `dfd-crop.png` into a realistic GCP-style architecture review DFD with the default 4-token flow-tuple standard and embedded LEGEND panel; Build Package 11A byte-refreshed `runs/RUN-001/inputs/` from the corrected sample (SHA-256 byte-equality verified across all 6 inputs). The precondition gate below (§Precondition Gate (Hard), gate 0) is **MET**. Downstream gates re-evaluate against the catalog / blueprint / template / sample inventory. BP13 (Diagrams) is now `next_allowed`. See `validation/package-10a-corrective-patch-checklist.md` and `validation/package-11a-input-refresh-checklist.md` for the falsifiable evidence.
