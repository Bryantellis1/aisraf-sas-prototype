<#
.SYNOPSIS
    Smoke-test the Build Package 01-12 surface of AISRAF SAS Prototype v0.1.2 (with corrective patch 10A on the sample side and 11A on the run side; BP12-SAMPLE-DFD-BLOCKER fully resolved; BP13 next_allowed).
...
if ($validationFails.Count -gt 0) {
    ...
    Add-Result -Status PASS -Check '11-validation-allowed' -Detail "validation/ contains only the Build Package 01-12 documents (plus Build Package 10A corrective-patch checklist, Build Package 11A run-input-refresh checklist, and the 6 Build Package 12C operator-experience planning artifacts)."
}
